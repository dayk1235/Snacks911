...
describe('Action System Unit Tests', () => {
  ...
  test('productFlow returns an upsell add_to_cart action', async () => {
    ...
    // The engine adds the product to cart and then suggests an UPSELL
    const upsellAction = response.actions?.find((a: Action) => a.type === 'add_to_cart');
    expect(upsellAction).toBeDefined();
    expect(upsellAction?.payload).toMatchObject({
