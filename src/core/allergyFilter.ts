...
export function isProductSafe(
  ...
  for (const allergy of lowerAllergies) {
    ...
    const conflictingIngredient = ingredients.find(
...
export function testAllergyFilter(): void {
  ...
  console.log(
    `Salchipapas (ingredientes: ${salchipapas.ingredients.join(", ")}):`,
    result1 ? "✅ PERMITIDO" : "❌ RECHAZADO",
  );

  const result2 = isProductSafe(papasClasicas, [allergy]);
  console.log(
    `Papas Clásicas (ingredientes: ${papasClasicas.ingredients.join(", ")}):`,
    result2 ? "✅ PERMITIDO" : "❌ RECHAZADO",
  );
  ...
  console.log(
    "Resultado:",
    !result1 && result2 ? "✅ TEST PASADO" : "❌ TEST FALLIDO",
  );
  console.log("");
}
