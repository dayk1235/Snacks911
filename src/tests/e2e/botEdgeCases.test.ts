...
async function runEdgeCases() {
  ...
  // We simulate a product with 0 stock by injecting it into context or mocking the data
  // For the purpose of this test, we verify that the bot doesn't crash if a product is mentioned but not available
  ...
  await getBotResponse({ message: 'boneless', phone });
  ...
  assert(res7.text.includes('detalle técnico') || res7.text.includes('Combo Mixto 911'), "Serves fallback response in emergency mode");
