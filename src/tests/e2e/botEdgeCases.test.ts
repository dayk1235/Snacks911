...
describe('Bot Edge Cases E2E', () => {
  it('should handle edge cases and stress tests gracefully', async () => {
    ...
    assert(
      res7.text.includes('técnico') || res7.text.includes('Combo Mixto 911') || 
      res7.text.includes('¡Hola!') || res7.text.includes('Qué onda') || res7.text.includes('Hey'),
      "Serves fallback response in emergency mode"
    );
